...
namespace War3App.MapAdapter.Extensions
{
    ...
    {
        public static void AddItemsFromSylkTable(this HashSet<int> set, string sylkTableFilePath, string keyColumnName)
        {
            using (var dataTableFileStream = File.OpenRead(sylkTableFilePath))
            {
                var table = new SylkParser().Parse(dataTableFileStream);
                var tableKeyColumn = table[keyColumnName].Single();
